 ...
 * each entry — offering ten widths mostly buys cache misses. These four cover a
 ...
 */
const WIDTHS = [240, 480, 800, 1280] as const;
 ...
 * to pick one.
 */
export function imageSrcSet(src: string, max: number = WIDTHS[WIDTHS.length - 1]): string | undefined {
 ...
 */
export function imageFallback(src: string): string {
  return imageUrl(src, 800);
}
