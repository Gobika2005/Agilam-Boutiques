  ...
  return (
    <div style={css('min-height:100%;background:#FBF6F2;padding-bottom:24px;')}>
      ...
      <div style={css('max-width:760px;')}>
        <div className="agx-eyebrow" style={css('font-size:10.5px;color:#B02454;margin:8px 24px 8px;')}>Frequently asked</div>
        <div style={css('margin:0 20px;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 12px 30px -20px rgba(107,20,54,.6);')}>
          {FAQS.map((f, i) => {
            const isOpen = open === i;
            return (
              <div key={f.q} style={css(`border-bottom:${i === FAQS.length - 1 ? 'none' : '1px solid #F5E4EC'};`)}>
                <button
                ...
                >
                  ...
                  <span style={css('flex:1;font-weight:700;font-size:13.5px;color:#2A1A20;')}>{f.q}</span>
        ...
        <div className="agx-eyebrow" style={css('font-size:10.5px;color:#B02454;margin:22px 24px 8px;')}>Still need help?</div>
        <div style={css('margin:0 20px;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 12px 30px -20px rgba(107,20,54,.6);')}>
          {[
            ...
            <a
              ...
              style={css(`display:flex;align-items:center;gap:13px;padding:14px 14px;text-decoration:none;color:inherit;border-bottom:${i === arr.length - 1 ? 'none' : '1px solid #F5E4EC'};`)}
            >
              ...
              <span style={css('flex:1;min-width:0;')}>
                ...
                <span style={css('display:block;font-size:12px;color:#8A7078;font-weight:600;margin-top:1px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;')}>{c.sub}</span>
              </span>
