...
export function Loading() {
  ...
  return (
    ...
    >
      {/* 512px WebP (27 kB). This is a splash screen — the one place a slow
          image is most obviously the wait itself — and it was loading the
          1.7 MB master PNG to draw it at 380px.
          The logo is transparent, so the depth comes from a drop-shadow filter
          that follows the artwork; a box-shadow would draw a rounded rectangle
          floating behind nothing. */}
      <img
        ...
        height={512}
        fetchPriority="high"
        style={css('width:min(72vw,380px);height:auto;filter:drop-shadow(0 24px 44px rgba(142,28,68,.34));animation:agxLogoIn 1s cubic-bezier(.2,.7,.2,1) both;')}
      />
