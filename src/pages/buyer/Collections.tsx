...
export function Collections() {
  ...
  /*
   * Every tile is now a real link to a real page.
   *
   * Colour and budget were the last two that called `setFilters()` and pushed
   * the buyer to the shared /shop URL — so those two collections could not be
   * shared, bookmarked, or ranked, while the other three had proper landing
   * pages. They now have their own (`/colours/red`, `/budget/under-3000`), which
   * is also what removed this screen's dependency on the global filter state.
   */
  ...
  return (
    <div style={css('min-height:100%;background:var(--ag-bg);padding-bottom:24px;')}>
      <DiscoveryHeader
        ...
        title="Shop by"
        accent="collection"
        subtitle="Every category, occasion, budget and colour our boutiques are listing right now. Each tile opens a page of its own."
        ...
        <>
          ...
          <div className="agx-occ-grid">
            {budgets.map((b) => (
              <Link
                key={b.maxPrice}
                to={routes.budget(b.maxPrice)}
                aria-label={`${b.label} — ${b.count} pieces`}
                className="agx-lift"
                style={css('display:flex;align-items:center;gap:12px;width:100%;padding:16px;border:1px solid var(--ag-surface-3);background:linear-gradient(140deg,var(--ag-surface),var(--ag-surface-2));border-radius:18px;cursor:pointer;text-align:left;text-decoration:none;color:inherit;font-family:inherit;box-shadow:0 14px 32px -28px rgba(107,20,54,.6);')}
              >
                <span style={css('min-width:0;flex:1;')}>
        ...
        <>
          ...
          <div className="agx-scroll" style={css('display:flex;gap:14px;overflow-x:auto;padding:2px 0 8px;')}>
            {colours.map((c) => (
              <Link
                key={c.name}
                to={routes.colour(c.name)}
                aria-label={`${c.name} — ${c.count} pieces`}
                className="agx-circle"
                style={css('flex:none;display:flex;flex-direction:column;align-items:center;gap:9px;padding:0;border:none;background:none;cursor:pointer;width:78px;text-decoration:none;color:inherit;')}
              >
